...
namespace E_Smart.Service
{
	public class VnPayService : IVnPayService
	{
		...
		public string CreatePaymentUrl(HttpContext context, VnPaymentRequestModel model)
		{
			...
			vnpay.AddRequestData("vnp_OrderInfo", "Chekout for order:" + model.OrderId);
			vnpay.AddRequestData("vnp_OrderType", "other"); //default value: other
			vnpay.AddRequestData("vnp_ReturnUrl", _config["VnPay:PaymentBackReturnUrl"]);

			vnpay.AddRequestData("vnp_TxnRef", tick); // Mã tham chiếu của giao dịch tại hệ thống của merchant. Mã này là duy nhất dùng để phân biệt các đơn hàng gửi sang VNPAY. Không được trùng lặp trong ngày

			//Tạo request checkout
			var paymentUrl = vnpay.CreateRequestUrl(_config["VnPay:vnp_Url"], _config["VnPay:HashSecret"]);

			return paymentUrl;

		}

		public VnPaymentResponseViewModel PaymentExecute(IQueryCollection collections)
		{
			var vnpay = new VnPayLibrary();
